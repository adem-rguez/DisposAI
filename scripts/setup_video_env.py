"""
Builds the dedicated Python 3.11 environment (`.venv-video/`) that
`scripts/video_diffusers_server.py` runs under for text/image-to-video
generation. Uses `uv` to fetch a standalone CPython 3.11 and pip (via
`uv pip`) to populate it — no system-wide Python 3.11 install required.

Safe to run under the system Python — this script only shells out to
`uv`/`pip`; it does not import any of the packages it installs.

Usage:
    python scripts/setup_video_env.py

Env vars (all optional):
    DISPOS_VIDEO_TORCH  torch version to install (default "2.6.0")
    DISPOS_VIDEO_CUDA   CUDA wheel tag from https://download.pytorch.org/whl/
                         (default "cu124")
"""

import os
import sys
from pathlib import Path

sys.path.insert(0, str(Path(__file__).resolve().parent))
from env_setup_common import ensure_uv, create_venv, uv_pip_install, stage, progress, resolve_venv_root

PROJECT_ROOT = Path(__file__).resolve().parent.parent
VENV_DIR = resolve_venv_root(PROJECT_ROOT) / ".venv-video"
VENV_PYTHON = VENV_DIR / ("Scripts/python.exe" if os.name == "nt" else "bin/python")
INSTALL_MARKER = VENV_DIR / ".install_complete"

TORCH_VERSION = os.environ.get("DISPOS_VIDEO_TORCH", "2.6.0")
CUDA_TAG = os.environ.get("DISPOS_VIDEO_CUDA", "cu124")
TORCH_INDEX_URL = f"https://download.pytorch.org/whl/{CUDA_TAG}"

STAGE_RESULTS = []  # (stage_label, ok: bool)


def _print_header(title):
    print(f"\n{'=' * 70}\n{title}\n{'=' * 70}")


def install_torch():
    uv_pip_install(
        VENV_PYTHON,
        [f"torch=={TORCH_VERSION}", "torchvision"],
        index_url=TORCH_INDEX_URL,
    )


def install_core_deps():
    # video_diffusers_server.py generically dispatches through
    # diffusers.DiffusionPipeline for any video architecture (Wan2.1,
    # CogVideoX, LTX-Video, Mochi-1, HunyuanVideo, AnimateDiff, SVD, ...);
    # transformers/accelerate are runtime deps of those pipelines even though
    # not imported by name here. imageio[-ffmpeg] is needed to encode the
    # generated frames to mp4.
    uv_pip_install(
        VENV_PYTHON,
        [
            "diffusers", "transformers", "accelerate", "safetensors",
            "pillow", "numpy<2", "imageio", "imageio-ffmpeg", "sentencepiece", "protobuf",
        ],
    )


def _run_stage(label, fn):
    progress(label, "running")
    ok = stage(label, STAGE_RESULTS)(fn)
    progress(label, "done" if ok else "error", message=None if ok else f"{label} failed")
    return ok


def print_summary():
    _print_header("Stage summary")
    for label, ok in STAGE_RESULTS:
        print(f"  {'OK    ' if ok else 'FAILED'} - {label}")
    print(
        f"\nVenv python: {VENV_PYTHON}\n"
        "The daemon auto-discovers this venv for the video (diffusers) backend."
    )


def main():
    ensure_uv()

    try:
        create_venv(VENV_DIR, python_version="3.11")
    except Exception as e:
        print(f"[FAILED] Could not create {VENV_DIR}: {e}", file=sys.stderr)
        sys.exit(1)

    core_ok = True
    core_ok &= _run_stage("torch (CUDA)", install_torch)
    core_ok &= _run_stage("core deps", install_core_deps)

    if core_ok:
        INSTALL_MARKER.write_text("ok")

    print_summary()


if __name__ == "__main__":
    main()
