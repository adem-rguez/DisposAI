"""
Builds the dedicated Python 3.11 environment (`.venv-sd/`) that
`scripts/diffusers_server.py` runs under for safetensors Stable Diffusion
image generation. Uses `uv` to fetch a standalone CPython 3.11 and pip (via
...
Usage:
    python scripts/setup_sd_env.py

Env vars (all optional):
    DISPOS_SD_TORCH  torch version to install (default "2.6.0")
    DISPOS_SD_CUDA   CUDA wheel tag from https://download.pytorch.org/whl/
                      (default "cu124")
"""
...
VENV_DIR = resolve_venv_root(PROJECT_ROOT) / ".venv-sd"
VENV_PYTHON = VENV_DIR / ("Scripts/python.exe" if os.name == "nt" else "bin/python")
INSTALL_MARKER = VENV_DIR / ".install_complete"

TORCH_VERSION = os.environ.get("DISPOS_SD_TORCH", "2.6.0")
CUDA_TAG = os.environ.get("DISPOS_SD_CUDA", "cu124")
...
def install_core_deps():
    # diffusers_server.py imports diffusers + PIL directly; transformers and
    # accelerate are runtime deps of most diffusers pipelines (text encoders,
    # low-memory device loading) even though not imported by name here.
    uv_pip_install(
        VENV_PYTHON,
        [
...
def print_summary():
    ...
    print(
        ...
        "The daemon auto-discovers this venv for the SD (diffusers) backend."
    )
